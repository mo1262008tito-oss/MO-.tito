...
const [selectedForensicData, setSelectedForensicData] = useState(null);
  ...
  // --- [3] المكونات الداخلية (Internal Components) ---
  ...
  const ExamBuilderUI = () => (
    <div className="exam-builder-container glass-card p-6">
      ...
      <div className="q-form">
          ...
          />
          {/* هنا تضع باقي حقول الخيارات */}
      </div>
      <button onClick={addQuestionToPool} className="titan-btn primary mt-4">إضافة السؤال للقائمة</button>
      <div className="questions-count">الأسئلة الحالية: {questions.length}</div>
    </div>
  );
  const AcademyUI = () => (
    <div className="academy-control-panel">
      ...
      {/* باقي محتوى الأكاديمية */}
      <button onClick={handleDeploy} className="titan-btn primary mt-4">نشر الكورس</button>
    </div>
  );
...
/**
 * [SUB-COMPONENT] FORENSIC MODAL
 * نافذة الفحص الجنائي الرقمي للطلاب
 */
const ForensicModal = ({ isOpen, onClose, data }) => {
  ...
  return (
    <div className="fixed inset-0 z-[500] flex items-center justify-center p-4 bg-black/90 backdrop-blur-xl">
      <motion.div 
        initial={{ scale: 0.9, opacity: 0 }}
        animate={{ scale: 1, opacity: 1 }}
        className="glass-card max-w-2xl w-full p-8 rounded-[2.5rem] border border-white/10 relative shadow-2xl bg-[#0a0a0a]"
      >
        <button 
          onClick={onClose} 
        ...
        <h3 className="text-2xl font-black text-white mb-6 flex items-center gap-3">
          <ShieldAlert className="text-red-500" /> سجل الفحص الجنائي الرقمي
        </h3>
        
        <div className="space-y-4">
          <div className="bg-red-500/5 border border-red-500/20 p-4 rounded-2xl text-red-400 text-sm italic">
            تحذير: هذه البيانات مستخرجة مباشرة من سجلات الأمان الخاصة بالخادم.
          </div>
          
          <pre className="bg-black/60 p-6 rounded-[1.5rem] text-xs font-mono text-green-500 overflow-auto max-h-[350px] custom-scrollbar border border-white/5">
            {data ? JSON.stringify(data, null, 2) : "// لا توجد محاولات اختراق مسجلة لهذا الحساب"}
          </pre>
        </div>
        
        <button 
          onClick={onClose}
          className="w-full mt-8 py-4 bg-white/5 hover:bg-white/10 text-gray-300 font-bold rounded-2xl transition-all"
        >
</button>
      </motion.div>
    </div>
